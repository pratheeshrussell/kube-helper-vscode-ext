...
import type { CRDListType, CRDTableItem, K8sCRD } from '@src/types/crd.type';
...
const getCRDList = () => {
    ...
    // CRDs are cluster-scoped, so no namespace needed for the command.
    // getNonNamespacedResourceByType expects {{resType}} and {{context}}
    // We need to ensure the command is just "kubectl get crds -o json" effectively
    // Forcing the command directly here as it's simpler for non-namespaced resources like CRD.
    const command = `kubectl get crds -o json`;

    tsvscode?.postMessage({
        type: MessageTypes.RUN_CMD_RESULT,
        subType: 'crdList', // Unique subtype for CRD list
        command: HelperUtils.prepareCommand(command, false) // Pass false to not inject namespace
    });
}

const gotoCRList = (crdData: CRDTableItem) => {
    // Navigate to the list of Custom Resources for this CRD
    // The route should handle crdName (which is crdData.name, e.g., certificates.cert-manager.io)
    // It also needs to pass along the group, version, kind, and scope for the CRList component.
    router.push({
        name: 'crlist', // This route will be defined in Router.ts
        params: {
            crdFullName: crdData.name, // Full name like 'certificates.cert-manager.io'
        },
        query: {
            group: crdData.group,
        ...
        try {
            ...
            if(list?.items){
                crdTableData.value = list.items.map((item: K8sCRD) => {
                    // Find the stored version to display (often there's only one, or one marked as storage)
                    const storedVersion = item.spec.versions.find(v => v.storage)?.name || item.spec.versions[0]?.name || 'unknown';
                    return {
                        uid: item.metadata.uid,
                        name: item.metadata.name, // Full name like 'certificates.cert-manager.io'
...
onMounted(() => {
    getCRDList();
    // CRDs are cluster-scoped, so breadcrumb is simpler
    globalStore.namespace = null; // Ensure namespace is cleared if coming from a namespaced view
    globalStore.breadcrumbItems = [{label: 'Custom Resource Definitions', navigateTo: 'crdlist', index: 0}];
});
</script>
